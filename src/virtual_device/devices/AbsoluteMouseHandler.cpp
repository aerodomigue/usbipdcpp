...
namespace usbipdcpp {
AbsoluteMouseHandler::AbsoluteMouseHandler(UsbInterface &handle_interface, StringPool &string_pool, int screen_width,
                                           int screen_height) :
...
int AbsoluteMouseHandler::get_screen_width() const {
    return screen_width_;
}
int AbsoluteMouseHandler::get_screen_height() const {
    return screen_height_;
}
int AbsoluteMouseHandler::get_screen_x1() const {
    return screen_x1_;
}
int AbsoluteMouseHandler::get_screen_y1() const {
    return screen_y1_;
}
int AbsoluteMouseHandler::get_screen_x2() const {
    return screen_x2_;
}
int AbsoluteMouseHandler::get_screen_y2() const {
    return screen_y2_;
}
...
void AbsoluteMouseHandler::on_new_connection(Session &current_session, error_code &ec) {
    ...
    send_thread_ = std::thread([this]() {
        while (!should_stop_) {
            ...
            state_cv_.wait(lock, [this]() { return state_changed_ || should_stop_; });
            if (should_stop_)
                break;
...
void AbsoluteMouseHandler::humanized_move(int from_x, int from_y, int to_x, int to_y, int duration_ms,
    ...
    while (t < 1.0 && !should_stop_) {
        ...
        std::int16_t x = static_cast<std::int16_t>(
                std::clamp(static_cast<int>(std::round(final_x)), 0, static_cast<int>(HID_MAX)));
        std::int16_t y = static_cast<std::int16_t>(
                std::clamp(static_cast<int>(std::round(final_y)), 0, static_cast<int>(HID_MAX)));
